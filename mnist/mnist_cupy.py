...
filename = 'log/mnist_w_reduce100_' + date + '.txt'
...
def reduce_data(data, label, rate):
    index = data.shape[0] - int(data.shape[0]*rate)
    data_ = data[:index]
    label_ = label[:index]
    return data_, label_
...
network = dense(784, 10)

print('--Start Generation--')
train_x, train_y = shuffle(train_x, train_y, random_state=random_state)
#train_y = label_noise(train_y, 1)
#train_x, train_y = reduce_data(train_x, train_y, 0.99)
train_x = train_x[:100]
train_y = train_y[:100]
n_batches = train_x.shape[0] // batch_size_ev
